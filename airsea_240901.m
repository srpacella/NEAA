...
%% Calculate median coastal stream sea-air pCO2 disequilibria for Pre- and Post-1990 (year-specific)
...
% 4. Calculate median DICdiseq pre and post 1990 for each site
...
for i = 1:length(names_current)
    ...
    dic_diseq_delta.(names_current{i}) = median(dic_diseq.(names_current{i})(index_post1990))-median(dic_diseq.(names_current{i})(index_pre1990));
    co2_diseq_delta.(names_current{i}) = median(co2_diseq.(names_current{i})(index_post1990))-median(co2_diseq.(names_current{i})(index_pre1990));

    dic_diseq_delta_list(i) = dic_diseq_delta.(names_current{i});
    co2_diseq_delta_list(i) = co2_diseq_delta.(names_current{i});
    
    pre1990_DIC_diseq(i) = median(dic_diseq.(names_current{i})(index_pre1990));
    post1990_DIC_diseq(i) = median(dic_diseq.(names_current{i})(index_post1990));
    pre1990_co2_diseq(i) = median(co2_diseq.(names_current{i})(index_pre1990));
    post1990_co2_diseq(i) = median(co2_diseq.(names_current{i})(index_post1990));
...
%% Make figures showing air-sea disequilibria
