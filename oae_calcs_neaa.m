%% SRP Update on 4/26/24 to check all calcs and (NOPE-->)calculate air-sea diseq using annual atmospheric Co2 values (not medians of current and baseline)
...
X = f_m;
Y_noisy_pCO2_airsea = airsea_allpast_estuary_pCO2';
plot_distribution_prctile(X,Y_noisy_pCO2_airsea,'Prctile',[25 50 75 90],'Color',[0 0 1]);
hold on
Y_noisy_pCO2_airsea = sum(airsea_allpast_estuary_pCO2'.* discharge_weight');
plot(X,Y_noisy_pCO2_airsea,'-k');
hold on
Y_noisy_pCO2_airsea = airsea_current_estuary_pCO2';
plot_distribution_prctile(X,Y_noisy_pCO2_airsea,'Prctile',[25 50 75 90],'Color',[1 0 1]);
hold on
Y_noisy_pCO2_airsea = sum(airsea_current_estuary_pCO2'.* discharge_weight');
plot(X,Y_noisy_pCO2_airsea,'--k');
ylabel('Sea-air \Delta{\itp}CO_2 (\muatm)')
grid on
box on
...
% Plot of discharge vs stream air-sea Deltapco2
figure
scatter(discharge_weight',airsea_current_estuary_pCO2(1,:)'-airsea_allpast_estuary_pCO2(1,:)')
figure
hist(airsea_current_estuary_pCO2(1,:)'-airsea_allpast_estuary_pCO2(1,:)',20)
...
% dicdiseq_all = [load_dicdiseq_pre1990',load_dicdiseq_post1990',load_dicdiseq_post1990'-load_dicdiseq_pre1990'];
% % Calculated discharged-weighted mean of DIC disequilibrium
% dicdiseq_fwa_pre1990= sum(discharge_weight'.*dicdiseq_all(:,1))
% dicdiseq_fwa_post1990= sum(discharge_weight'.*dicdiseq_all(:,2))
% dicdiseq_fwa_diff= sum(discharge_weight'.*dicdiseq_all(:,3))
% 
% dicdiseq_fwa_all = [dicdiseq_fwa_pre1990;dicdiseq_fwa_post1990;dicdiseq_fwa_diff];
% figure
% boxchart(dicdiseq_all,'MarkerStyle','none')
% hold on
% %plot(dicdiseq_fwa_all,'-o')
% grid on
% ylabel('Stream DIC disequilibrium (\muatm)')
